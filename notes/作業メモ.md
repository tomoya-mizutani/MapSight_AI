...
09:56
データ取得がうまくいっていないのはわかるんだけど，
前回どういう操作をして，データ取得したのか？のメモが見つからなくて困っている
スケルトン実装をかましたことで，自分が理解できていないとこで変な挙動が起きているんだと思うんだけど，

課題分析と把握が今日のタスクになりそう...

あと，ToolsのDoc管理してもらおう
チャッピーに与えるプロンプト
”docs内にToolsフォルダを作成し，tools内にあるPythonソースコードを解析し，それぞれのドキュメントファイルをmarkdownで作成してください．”


memo 
大きく実装のドメインを分ける
・画像データの取得（Youtubeからのダウンロード）
・画像データから位置情報などの抽出
・データ管理に登録⇐　ここがよくわかっていない．データ量が大きいときにどうやって対応させるか？スケールのｎ仕方がいまいち．．．
・データ整備
・データ分析
・モデル学習
・評価
...
画像データから色の情報を基に位置情報を抽出するサンプルプログラムを出力させた
取り敢えず，精度は悪いけど，色ごとに位置情報を取得することはできた．
次の課題は，その精度を上げること，
ちゃんと色抽出アルゴリズムを見直したほうが良さげ，



今まで作成したけど，よく理解していないコードを捨てる作業を行う．
どうせ上手く実行できていないし，まぁ最悪Gitの中で掘り起こせばいいし．．

残すのは　ドキュメントファイルとデータ，あとはJson
これはシステムの設計思想だけは引き継ぎたいため，

ドキュメントがうまくまとまってなくてもいいので，
取り敢えず一次情報としてすべて残す．

新しく"Remake”ブランチを作成し，その中で，期待通りの挙動をしていないソースコードを削除した．

PROJCE_STATIS to
QUICK_GUIDE
の二つのマークダウンファイルを削除した．（中に何も記述されていなかったため，

Google Colab関連のソースコードをnotebookディレクトリにぶち込んだ
config.yamlにコメントを追加した．
課題.mdファイルを追加した．このファイルに上手く行かなったことを書き殴っていく．

05:20　作業終了

## 2026/01/04

crop_minimap.pyに関して，入出力ディレクトリのハードコーディングを解消した．

segment_rounds_update.pyの問題点を課題.mdに追加した．
IssueにおけるTask（2.3.18.36）あたりが該当する．

/home/mizutani-m2/workspace/MapSight_AI/erangel_players.json
において，チームカラー(bgr)の検知と座標の抽出が行えていることを確認した．（精度は未確認．）<br>クラスター分類しているっぽいとに指揮している
